...
!  real(r_kind) dz,denom,jqr_num,jqli_num,jqr,jqli !modified
  ...
  real(r_kind) rdBZ,presw,dbznoise,rSPD
...
!    Gross error checks
     ...
     if( abs(ddiff) >= 10.0 .and.  sqrt( rSPD + 25.0 ) / abs(ddiff) < CR_ratio )then
        jqg = 1.0_r_kind
     end if
   
     jqr = 0.0_r_kind
     if(  abs(ddiff) >= 10.0 .and. data(idbzob,i) .ge. distinguish_storm ) jqr = 1.0_r_kind
     if(  abs(ddiff) >= 10.0 .and. data(idbzob,i) .lt. distinguish_storm ) jqr = -1.0_r_kind
